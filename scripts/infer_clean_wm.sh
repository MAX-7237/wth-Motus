...
CONFIG_FILE="${CONFIG_FILE:-configs/action_following_lerobot_rot6d20_clean_only.yaml}"
CKPT="${CKPT:-}"
IMAGE="${IMAGE:-${1:-}}"
...
if [ -z "$IMAGE" ] || [ -z "$INSTRUCTION" ] || [ -z "$ACTIONS" ]; then
    echo "Usage: IMAGE=/path/to/input.png INSTRUCTION='task text' ACTIONS=/path/to/actions.npy bash scripts/infer_clean_wm.sh"
    echo "   or: bash scripts/infer_clean_wm.sh /path/to/input.png 'task text' /path/to/actions.npy"
